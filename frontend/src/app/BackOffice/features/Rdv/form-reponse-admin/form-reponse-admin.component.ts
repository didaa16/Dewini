...
@Component({
  selector: 'app-form-reponse-admin',
  templateUrl: './form-reponse-admin.component.html',
  styleUrls: ['./form-reponse-admin.component.css']
})
export class FormReponseAdminComponent {
  ...
  submitForm(): void {
    ...
    this.reponseService.createReponseWithSms(reponse).subscribe({
      next: () => {
        ...
        setTimeout(() => {
          this.router.navigate(['/liste-reponses']);
        }, 2000);
      },
      error: (err) => {
